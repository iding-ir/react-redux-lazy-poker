export const NUMBER_OF_DEFAULT_PLAYERS = 2;
export const MINIMUM_NUMBER_OF_PLAYERS = 1;
export const NUMBER_OF_CARDS_PER_PLAYER = 2;
export const NUMBER_OF_CARDS_FOR_FLOP = 3;
export const NUMBER_OF_CARDS_FOR_TURN = 1;
export const NUMBER_OF_CARDS_FOR_RIVER = 1;

export const FLUSH_CONDITION = 5;
export const STRAIGHT_CONDITION = 5;
export const PAIR_CONDITION = 2;
export const THREE_OF_A_KIND_CONDITION = 3;
export const FOUR_OF_A_KIND_CONDITION = 4;

export const AUTOPLAY_DELAY_MS = 2000;

export const RANDOM_PLAYER_NAMES = ["Koshka", "Gulbi", "Remuss"];
